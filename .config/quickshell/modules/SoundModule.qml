...
PopupWindow {
    ...
    // Обновляем позицию каждую секунду пока играет
    Timer {
        interval: 1000
        ...
        ColumnLayout {
                        ...
                        Text {
                            ...
                            MouseArea {
                                ...
                                onClicked: if (root.spotify)
                                    root.spotify.shuffle = !root.spotify.shuffle
                            }
                        }
                        Text {
                            ...
                            color: root.spotify && root.spotify.loopState !== MprisLoopState.None ? root.colGreen : root.colMuted
                            MouseArea {
                                anchors.fill: parent
                                cursorShape: Qt.PointingHandCursor
                                onClicked: {
            ...
            Rectangle {
                ...
                readonly property real ratio: (root.hasPlayer && root.spotify && root.spotify.lengthSupported && root.spotify.length > 0)
                    ? Math.min(root.spotify.position / root.spotify.length, 1.0)
                    : 0.0

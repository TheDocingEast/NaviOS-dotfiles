...
PopupWindow {
    ...
    // ── Hover state — пробрасывается из shell.qml ─────────────────────────
    // Когда мышь на кнопке в баре — выставить true, убрать — false.
    // PopupWindow сам следит за своей мышью через panelHovered.
    ...
    // ── Spotify ───────────────────────────────────────────────────────────
    // Явно читаем .values чтобы binding реагировал на добавление/удаление плееров
    readonly property var spotify: {
        var players = Mpris.players.values   // <-- реактивная зависимость на весь список
        for (var i = 0; i < players.length; i++) {
            var p = players[i]
            var dbus = (p.dbusName    || "").toLowerCase()
            var id   = (p.identity    || "").toLowerCase()
            var de   = (p.desktopEntry|| "").toLowerCase()
            if (dbus.includes("spotify") || id.includes("spotify") || de.includes("spotify"))
                return p
        }
        return null
    }
    ...
    readonly property bool playing:
        hasPlayer && spotify.playbackState === MprisPlaybackState.Playing
    ...
    onVisibleChanged: {
        if (visible) {
            ...
            card.opacity = 0
            anim.restart()
            if (root.spotify) root.spotify.positionChanged()
    ...
    // Эмитим positionChanged каждую секунду пока играет
    Timer {
        interval: 1000; running: root.visible && root.playing; repeat: true
        onTriggered: {
            if (root.spotify) root.spotify.positionChanged()
            progressBar.requestPaint()
        }
    ...
    Rectangle {
        ...
        // Следим за мышью внутри попапа — поверх всех дочерних элементов
        MouseArea {
            anchors.fill: parent
            hoverEnabled: true
        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                Text {
                    ...
                    color: root.volLevel > 90 ? root.colRed
                         : root.volLevel > 50 ? root.colYellow
                         : root.colCyan
                }
                ...
                Rectangle {
                    ...
                    MouseArea {
                        ...
                        onClicked: (mouse) => {
                            root.volChanged(Math.max(0, Math.min(100,
            ...
            RowLayout {
                ...
                Rectangle {
                    ...
                    // Внутренний Rectangle с тем же радиусом — гарантирует скругление
                    Rectangle {
                        anchors.fill: parent
                        radius: 10
                        clip: true
                    ...
                    Text {
                        ...
                        text: "󰓇"
                        font { pixelSize: 30; family: root.fontFamily }
                        color: root.colFg
                    }
                    Rectangle {
                            ...
                            MouseArea {
                                ...
                                onClicked: {
                                    ...
                                    if (s === MprisLoopState.None)
                                        root.spotify.loopState = MprisLoopState.Playlist
                                    else if (s === MprisLoopState.Playlist)
                                        root.spotify.loopState = MprisLoopState.Track
                                    else
                                        root.spotify.loopState = MprisLoopState.None
                                }
                            }
                        }
                    }
            ...
            Canvas {
                ...
                Connections {
                    target: root.spotify
                    function onPositionChanged() { progressBar.requestPaint() }
                }
                ...
                onPaint: {
                    ...
                    ctx.fillStyle = Qt.rgba(0.85, 0.87, 0.91, 0.25)  // colFg приглушённый трек
                    ctx.fill()
                    if (root.hasPlayer && root.spotify.lengthSupported && root.spotify.length > 0) {
                        var ratio = Math.min(root.spotify.position / root.spotify.length, 1.0)
                        if (ratio > 0) {
                            ctx.beginPath()
                            ctx.roundRect(0, 0, width * ratio, height, 2)
                            ctx.fillStyle = Qt.rgba(0.85, 0.87, 0.91, 1.0)  // colFg полный
                            ctx.fill()
                        }
                    }
                }
    ...
    component CtrlBtn: Item {
        ...
        property int    btnW:      36; property int btnH: 36
        ...
        Rectangle {
            anchors.fill: parent; radius: parent.btnRadius
        ...
        Text {
            anchors.centerIn: parent
            text: parent.btnText
            font { pixelSize: parent.btnSize; family: root.fontFamily }
            color: root.colFg
        }
        MouseArea {
    ...
    function formatTime(sec) {
        var s = Math.floor(sec); var m = Math.floor(s / 60); s = s % 60
        return m + ":" + (s < 10 ? "0" : "") + s
    }
}